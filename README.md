...
<details>
  <summary><h4 >프로젝트를 마치고.. </h4></summary>

👤 [@menzzi](https://github.com/menzzi)
100만 건 이상의 데이터를 다루며 단순한 삽입 작업조차 쉽지 않다는 것을 실감했고, 속도 저하, OS 저장공간 부족, Colab 협업 문제 등 다양한 제약을 겪었습니다. 이를 과정에서 효율적인 작업 환경 구성과 협업을 고려한 실무형 데이터 처리 역량의 중요성을 체감했습니다.

👤[@ddddabi](https://github.com/ddddabi)
프로젝트 초반에는 순조로웠지만 진행할수록 예상치 못한 트러블이 많이 발생했고, 이를 해결하며 트러블슈팅 내용을 체계적으로 문서화해두는 것이 이후 프로젝트의 효율성과 완성도를 높이는 데 얼마나 중요한지 깨닫는 경험이 되었습니다.

👤[@ChatHongPT](https://github.com/ChatHongPT)
이번 프로젝트를 통해 단순히 파티셔닝을 이해하는 수준을 넘어서, 직접 다양한 파티셔닝 전략을 적용하고 성능을 수치로 비교해보며, 실제 설계가 시스템 성능에 미치는 영향을 체감할 수 있었습니다. 특히 쿼리 패턴에 따라 어떤 파티셔닝이 적절한지 고민하고 검증하는 과정을 통해 실무적인 감각도 키울 수 있었던 의미 있는 경험이었습니다.

👤[@imewuzin](https://github.com/imewuzin)
어떤 프로젝트를 할지 함께 고민하던 중, 소상공인에게 실질적으로 도움이 되는 서비스를 만들어보자는 목표로 기획을 시작하게 되었습니다. 백만 건에 이르는 대규모 데이터를 대상으로 다양한 파티셔닝 기법을 적용하고 성능을 비교해보며, 데이터의 특성에 따라 적합한 파티셔닝 방안을 탐색할 수 있었습니다. 현재는 단순한 대시보드를 제공하는 수준에 머물러 있지만, 앞으로는 실시간 데이터를 연계해 소상공인이 실제로 활용할 수 있는 유용한 서비스로 발전시키고자 합니다.
</details>
